...
module.exports = function factory(provider) {
    return async function validateJWT(ctx, next) {
        ...
        const client = await provider.Client.find(payload.iss, {fresh: true});
        ...
        try {
            debug("validate keystore %O", client.jwks);
            const keyStore = await jose.JWK.asKeyStore(client.jwks);

            debug("validate jwt %O", ctx.oidc.assertion_grant.body);
